...
{
    return shp_header.shape_type == SHPT_POLYGONM;
}

static int
test_record_shape_type(void)
{
    return shp_record->shape_type == SHPT_POLYGONM;
}

static int
test_bbox(void)
{
    const shp_box_t *b = &polygonm->box;
    return b->x_min == 1 && b->y_min == 1 && b->x_max == 2 && b->y_max == 2;
}
